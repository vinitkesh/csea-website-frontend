...
export function formatEvent(item) {
	...
	event.gallery_url = process.env.NEXT_PUBLIC_BACKEND_URL + (event?.gallery?.data?.attributes?.url)
...
export function formatImages(item){
	const image = {
		id: item?.id,
		thumbnail: process.env.NEXT_PUBLIC_BACKEND_URL + (item?.attributes?.formats?.thumbnail?.url),
		url: process.env.NEXT_PUBLIC_BACKEND_URL +(item?.attributes?.url)
	}
...
export function formatGallery(item){
	...
	gallery.cover_img =
		process.env.NEXT_PUBLIC_BACKEND_URL +
		(gallery?.event?.data?.attributes?.cover?.data?.attributes?.formats?.thumbnail?.url ?? gallery?.event?.data?.attributes?.cover?.data?.attributes?.url)

	gallery.gallery_url = process.env.NEXT_PUBLIC_BACKEND_URL + (gallery?.event?.data?.attributes?.gallery?.data?.attributes?.url)
